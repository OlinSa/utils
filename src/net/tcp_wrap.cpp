...
int TCPWrap::getsockopt(int level, int optname, void *optval, socklen_t *optlen)
{
    return ::getsockopt(fd, level, optname, optval, optlen);
}
int TCPWrap::setsockopt(int level, int optname, void *optval, socklen_t optlen)
{
    return ::setsockopt(fd, level, optname, optval, optlen);
}
ssize_t TCPWrap::write(void *buf, ssize_t len)
{
    return ::send(fd, buf, len, 0);
}
ssize_t TCPWrap::read(void *buf, ssize_t count)
{
